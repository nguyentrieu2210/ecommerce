            ...
            <form method="GET" action="/{{ Request::path() }}">
                <div class="filter-advanced flex-space-between ">
                    ...
                    <div class="filter-advanced-r flex">
                        <select name="statusPayment" class="setupSelect2 ml">
                            @foreach (__('filter.statusPayment') as $key => $val)
                                <option {{ Request::query('statusPayment') == $key ? 'selected' : '' }}
                                    value="{{ $key }}">{{ $val }}</option>
                            @endforeach
                        </select>
                        <select name="statusReceive" class="setupSelect2 ml">
                            @foreach (__('filter.statusReceive') as $key => $val)
                                <option {{ Request::query('statusReceive') == $key ? 'selected' : '' }}
                                    value="{{ $key }}">{{ $val }}</option>
                            @endforeach
                        </select>
